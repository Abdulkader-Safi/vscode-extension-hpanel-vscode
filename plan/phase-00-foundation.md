...
- [ ] Update `package.json` — `name: "hostinger-vps-manager"`, `displayName: "Hostinger VPS Manager"`, `description` from PRD §1, `version: "0.1.0"` (pre-release), `publisher: <to-be-set>`, `repository`, `bugs`, `homepage`, `license: "MIT"` — _file: `package.json`_
  - Acceptance: `npm pack --dry-run` reports the new name; `vsce ls` (if installed) shows the renamed manifest.
- [ ] Set marketplace categories to `["Other", "Azure"]` → reduce to just `["Other"]` for v0.1; revisit in Phase 10 — _file: `package.json`_
- [ ] Set `engines.vscode` to `^1.85.0` per PRD §2.1 (downgrade from current `^1.106.1` so older VS Code installs aren't excluded) — _file: `package.json`_
  - Acceptance: PRD §2.1 minimum honored; `vsce` does not warn.

## Commands

- [ ] Replace `contributes.commands` with the v1.0 set — _file: `package.json`_
...
- [ ] Replace command id `svelte-starter.openSvelteView` with `hostinger-vps.open` and register all three commands — _file: `src/extension.ts`_
- [ ] Rename class `SvelteWebviewProvider` → `HostingerWebviewProvider`; rename file `src/WebviewProvider.ts` → `src/HostingerWebviewProvider.ts` — _files: `src/extension.ts`, `src/HostingerWebviewProvider.ts`_
- [ ] Update panel id `"svelteView"` → `"hostinger-vps.panel"` and title `"Svelte App"` → `"Hostinger VPS Manager"` — _file: `src/HostingerWebviewProvider.ts`_
- [ ] Delete demo pages and routes — _files: `src/webview/pages/NotificationPage.svelte`, `src/webview/pages/DirectoryListPage.svelte`, `src/webview/App.svelte`_
  - Acceptance: `grep -ri "Notification\|DirectoryList" src` returns no matches.
- [ ] Remove demo message handlers `showNotification`, `getDirectoryContents` — _file: `src/HostingerWebviewProvider.ts`_

## Documentation

- [ ] Replace `README.md` with a stub for the product (full marketplace README lands in Phase 10) — _file: `README.md`_
  - Include: product name, one-line summary from PRD §1, "In active development" note, link to `PRD.md`, link to `plan/`.
- [ ] Reset `CHANGELOG.md` with `## [Unreleased] — Initial scaffolding` entry — _file: `CHANGELOG.md`_
- [ ] Add `LICENSE` (MIT, per PRD §2.1) — _file: `LICENSE`_
