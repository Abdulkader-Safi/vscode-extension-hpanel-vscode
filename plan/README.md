...
⁵ Phase 5: actions inlined in `ProjectRow.svelte` (no separate `projectActions.ts`). `NewProjectModal` URL-input mode deferred — paste-YAML only ships in v1.0. Env-var key sanitation regex and component-level tests (filter, auto-scroll) deferred to Phase 9 polish.
